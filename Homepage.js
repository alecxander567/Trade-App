      ...
      <View style={styles.appWrapper}>
        <View style={styles.navbar}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <TouchableOpacity
              style={[{ marginRight: 20 }, route.name === 'ProfileScreen' && { opacity: 0.7 }]}
              onPress={() => navigation.navigate('ProfileScreen')}
            >
              <Ionicons
                name={route.name === 'ProfileScreen' ? 'person-circle' : 'person-circle-outline'}
                size={28}
                color={route.name === 'ProfileScreen' ? '#FFD700' : '#fff'}
              />
            </TouchableOpacity>
          ...
          {items.length === 0 ? (
            ...
            items.map((item, index) => (
              ...
              >
                ...
                >
                  Posted by -{' '}
                  {item.owner === currentUserId || item.owner?._id === currentUserId
                    ? 'You'
                              ...
                              onPress: async () => {
                                try {
                                  const response = await fetch(
                                    `http://192.168.1.99:5000/api/items/${item._id}`,
                                    {
                                      method: 'DELETE',
                                      headers: { 'Content-Type': 'application/json' },
                ...
                >
                  <TouchableOpacity
                    ...
                    onPress={async () => {
                      ...
                      try {
                        const response = await fetch(
                          `http://192.168.1.99:5000/api/items/${item._id}/star`,
                          {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ userId: currentUserId }),
